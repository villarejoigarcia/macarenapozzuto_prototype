...
function ScrollItem({
  ...
  return (
    <div className={`w-full relative transition-height ${classDuration} ease-[cubic-bezier(0.42,0,0.58,1)] ${desktopHeightClass}`}>
      <div
        ref={ref}
        className={`item w-full h-full relative flex justify-center ${(isActive && isExpanded) || (isHover && !isMobile && !isExpanded) ? 'overflow-x-auto cursor-ew-resize' : 'overflow-hidden cursor-pointer'} ${isExpanded ? 'pointer-events-auto' : 'pointer-events-none'}`}
        onClick={() => {
            handleClick();
            resetCurrentScroll();
            if (!isMobile) return;
      ...
      >
        <motion.div
          ...
          className="origin-center lg:w-fit h-full will-change-transform flex justify-center pointer-events-auto"
          // onClick={() => {
          //   handleClick();
          //   resetCurrentScroll();
          //   if (!isMobile) return;
